...
RC Table::make_record(int value_num, const Value *values, char * &record_out) {
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if(value.type==DATES)
    {
      ...
      int res=*(int*)value.data;
      int y=res/10000;
      int m=res/100%100;
      int d=res%100;
      LOG_INFO("DATE IS%d,%d,%d",y,m,d);
      if(y>2038||y<1970)
      {
        return RC::SCHEMA_FIELD_TYPE_MISMATCH;
      }
      if(y==2038&&m>2){return RC::SCHEMA_FIELD_TYPE_MISMATCH;}
      int d1[12]={31,29,31,30,31,30,31,31,30,31,30,31};
      if(d>d1[m]){return RC::SCHEMA_FIELD_TYPE_MISMATCH;}
      if((y%4!=0)&&(m==2)&&(d==29)){return RC::SCHEMA_FIELD_TYPE_MISMATCH;}
    }
  }
